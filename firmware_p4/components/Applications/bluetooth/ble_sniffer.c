...
esp_err_t ble_sniffer_start(void) {
  ...
  sniffer_task_stack = (StackType_t *)heap_caps_malloc(
  ...
  sniffer_queue_struct =
      (StaticQueue_t *)heap_caps_malloc(sizeof(StaticQueue_t), MALLOC_CAP_SPIRAM);
  sniffer_queue_storage =
      (uint8_t *)heap_caps_malloc(SNIFFER_QUEUE_SIZE * sizeof(sniffer_packet_t), MALLOC_CAP_SPIRAM);
