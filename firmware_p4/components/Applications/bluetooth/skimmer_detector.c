...
skimmer_record_t *skimmer_detector_get_results(uint16_t *count) {
  ...
  if (remote_count > MAX_SKIMMERS_FOUND)
    remote_count = MAX_SKIMMERS_FOUND;

  if (remote_count < cached_count)
    cached_count = 0;

  if (remote_count > cached_capacity) {
    skimmer_record_t *new_buf =
        (skimmer_record_t *)realloc(cached_results, remote_count * sizeof(skimmer_record_t));
    if (!new_buf) {
      if (count)
        *count = cached_count;
      return cached_results ? cached_results : &empty_record;
