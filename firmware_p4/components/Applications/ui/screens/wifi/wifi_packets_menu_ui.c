...
static void nav_timer_cb(lv_timer_t * t) {
    if (lv_screen_active() != screen_wifi_packets_menu) {
        lv_timer_delete(t);
        nav_timer = NULL;
        return;
    }
...
void ui_wifi_packets_menu_open(void) {
    if (screen_wifi_packets_menu) { lv_obj_del(screen_wifi_packets_menu); screen_wifi_packets_menu = NULL; }

    screen_wifi_packets_menu = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(screen_wifi_packets_menu, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(screen_wifi_packets_menu, LV_OPA_COVER, 0);
    lv_obj_remove_flag(screen_wifi_packets_menu, LV_OBJ_FLAG_SCROLLABLE);

    menu = menu_component_create(screen_wifi_packets_menu, "PACKETS", "/assets/icons/wifi_menu_icon.bin");
