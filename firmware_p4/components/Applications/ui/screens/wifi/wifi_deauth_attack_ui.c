...
typedef enum { DEAUTH_VIEW_APS = 0, DEAUTH_VIEW_ATTACK = 1, DEAUTH_VIEW_CLIENTS = 2 } deauth_view_t;
...
static void update_attack_labels(void) {
  ...
  if (lbl_client) {
    if (!mode_broadcast && has_client) {
      lv_label_set_text_fmt(lbl_client,
  ...
  if (btn_attack) {
    lv_label_set_text(lv_obj_get_child(btn_attack, 0),
                      is_attacking ? "ATTACKING..." : "START ATTACK");
    lv_obj_set_style_bg_color(
        btn_attack, is_attacking ? current_theme.bg_item_top : current_theme.border_accent, 0);
  }
}
...
static void start_attack(void) {
  if (mode_broadcast) {
    if (!wifi_deauther_start(&selected_ap, WIFI_DEAUTHER_TYPE_INVALID_AUTH, true)) {
    ...
    if (!wifi_deauther_start_targeted(
            &selected_ap, selected_client, WIFI_DEAUTHER_TYPE_INVALID_AUTH)) {
      msgbox_open(LV_SYMBOL_CLOSE, "Failed to start attack", "OK", NULL, NULL);
      return;
    }
  }
...
static void show_attack_view(void) {
  ...
  if (lbl_target)
    lv_obj_del(lbl_target);
  if (lbl_mode)
    lv_obj_del(lbl_mode);
  if (lbl_client)
    lv_obj_del(lbl_client);
  if (btn_attack)
    lv_obj_del(btn_attack);
  if (lbl_packets)
    lv_obj_del(lbl_packets);
