...
typedef enum { PMKID_VIEW_APS = 0, PMKID_VIEW_SCAN = 1 } pmkid_view_t;
...
static void show_scan_view(void) {
  ...
  if (wifi_sniffer_start_stream(WIFI_SNIFFER_TYPE_PMKID, selected_ap.primary, NULL)) {
    is_scanning = true;
  } else {
    is_scanning = false;
