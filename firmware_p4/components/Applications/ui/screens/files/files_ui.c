...
static void update_scroll_bar(void) {
  if (!scroll_bar || entry_count <= 1)
    return;
  int32_t pos = track_y_start + (selected * (track_h - 20)) / (entry_count - 1);
...
static void build_file_list(void) {
  ...
  if (!folder_dsc)
    folder_dsc = assets_get("/assets/frames/folder_frame_0.bin");
  if (!file_dsc)
    file_dsc = assets_get("/assets/frames/file_frame_0.bin");
...
void ui_files_open(void) {
  ...
  if (!folder_icon)
    folder_icon = assets_get("/assets/frames/folder_frame_0.bin");
  if (folder_icon) {
  ...
  if (sb_dsc)
    lv_image_set_src(scroll_bar, sb_dsc);
