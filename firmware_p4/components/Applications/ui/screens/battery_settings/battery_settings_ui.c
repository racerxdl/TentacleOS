...
void ui_battery_settings_open(void) {
  ...
  menu_component_add_toggle(&menu, "/assets/icons/battery_menu_icon.bin", "PWR SAVE", power_save);
  menu_component_add_selector(&menu, NULL, "TIMEOUT", timeout_options[timeout_idx]);
  menu_component_add_selector(&menu, NULL, "MODE", perf_options[perf_idx]);
