...
static uint64_t dec_varint(const uint8_t *buf, uint16_t max_len, uint16_t *out_consumed)
{
    ...
    if (out_consumed != NULL) *out_consumed = i;
    return value;
}
                   ...
                   uint32_t *out_request_id)
{
    ...
    *out_payload = NULL;
    *out_payload_len = 0;
    if (out_request_id != NULL) *out_request_id = 0;
    ...
    while (i < data_len) {
        ...
        if (wire_type == 0) {
            ...
            i += used;
            if (field == 1) *out_portnum = (uint32_t)value;
        } else if (wire_type == 2) {
            uint16_t used = 0;
            uint32_t length = (uint32_t)dec_varint(&data_bytes[i], data_len - i, &used);
            i += used;
            if (i + length > data_len) return false;
            if (field == 2) {
            ...
            if (i + 4 > data_len) return false;
            if (field == 6 && out_request_id != NULL) {
                *out_request_id = ((uint32_t)data_bytes[i]) |
                                   ((uint32_t)data_bytes[i + 1] << 8) |
                                   ((uint32_t)data_bytes[i + 2] << 16) |
                                   ((uint32_t)data_bytes[i + 3] << 24);
            }
            i += 4;
        } else {
            return false;
...
esp_err_t mt_modules_init(uint32_t node_num)
{
    ...
    ESP_LOGI(TAG, "Initialized - 9 modules (Admin, NodeInfo, Position, Text, Routing, TraceRoute, Telemetry, KeyVerify, NeighborInfo)");
    return ESP_OK;
}
                         ...
                         const uint8_t *data_bytes, uint16_t data_len)
{
    ...
    if (!mt_parse_data(data_bytes, data_len,
                        &portnum, &payload, &payload_len, &request_id)) {
    ...
    if (meta->from != 0 && meta->from != s_our_node &&
