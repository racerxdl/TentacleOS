                              ...
                              bool is_compressed)
{
    if (has_seen_recently(meta->id)) {
        ESP_LOGD(TAG, "Duplicate text id=0x%08lX, dropping",
    ...
    if (is_compressed) {
        ...
        if (out_len <= 0 || out_len >= (int)sizeof(text)) {
            ESP_LOGW(TAG, "Unishox2 decompress failed from 0x%08lX (in=%u out=%d)",
                     (unsigned long)meta->from, len, out_len);
            return;
        }
        text[out_len] = 0;
        text_len = (uint16_t)out_len;
        ESP_LOGI(TAG, "Text[uncompressed %u->%u] from 0x%08lX ch=%u RSSI=%d SNR=%d: \"%s\"",
                 len, text_len,
                 (unsigned long)meta->from, meta->channel,
                 meta->rssi_dbm, meta->snr_db, text);
    } else {
        ...
        text[copy_len] = 0;
        text_len = copy_len;
        ESP_LOGI(TAG, "Text from 0x%08lX ch=%u RSSI=%d SNR=%d: \"%s\"",
                 (unsigned long)meta->from, meta->channel,
                 meta->rssi_dbm, meta->snr_db, text);
    }
    (void)text_len;
