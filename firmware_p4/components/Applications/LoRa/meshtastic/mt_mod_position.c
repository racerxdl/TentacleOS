...
void mt_mod_position_init(uint32_t node_num) {
  ...
  if (read_len == sizeof(stored)) {
    ...
    ESP_LOGI(TAG,
             "Fixed position loaded - lat=%.6f lon=%.6f alt=%ldm",
             s_lat_e7 / 1.0e7,
             s_lon_e7 / 1.0e7,
             (long)s_alt_m);
  } else {
...
void mt_mod_position_on_received(const mt_packet_meta_t *meta,
                                 ...
                                 uint16_t len) {
  (void)payload;
  ESP_LOGI(TAG, "Position received from 0x%08lX (%u bytes)", (unsigned long)meta->from, len);
...
void mt_mod_position_tick(uint32_t now_s) {
  ...
  meshtastic_mesh_send_data(MT_POSITION_BCAST_ADDR,
                            0,
                            MT_POSITION_HOP_LIMIT,
                            MT_PORT_POSITION,
                            pos,
