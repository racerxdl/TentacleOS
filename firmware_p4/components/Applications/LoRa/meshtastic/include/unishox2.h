...
/// Default Horizontal codes. When composition of text is know beforehand, the other hcodes in this
...
/// Default preset parameter set. When composition of text is know beforehand, the other parameter
/// sets in this section can be used to achieve more compression.
...
#define USX_PSET_ALPHA_ONLY \
  USX_HCODES_ALPHA_ONLY, USX_HCODE_LENS_ALPHA_ONLY, USX_FREQ_SEQ_TXT, USX_TEMPLATES
...
#define USX_PSET_FAVOR_DICT \
  USX_HCODES_FAVOR_DICT, USX_HCODE_LENS_FAVOR_DICT, USX_FREQ_SEQ_DFLT, USX_TEMPLATES
/// Preset parameter set favouring symbols
#define USX_PSET_FAVOR_SYM \
  USX_HCODES_FAVOR_SYM, USX_HCODE_LENS_FAVOR_SYM, USX_FREQ_SEQ_DFLT, USX_TEMPLATES
...
#define USX_PSET_NO_DICT \
  USX_HCODES_NO_DICT, USX_HCODE_LENS_NO_DICT, USX_FREQ_SEQ_DFLT, USX_TEMPLATES
...
#define USX_PSET_NO_UNI_FAVOR_TEXT \
  USX_HCODES_NO_UNI, USX_HCODE_LENS_NO_UNI, USX_FREQ_SEQ_TXT, USX_TEMPLATES
...
#define USX_PSET_JSON_NO_UNI \
  USX_HCODES_NO_UNI, USX_HCODE_LENS_NO_UNI, USX_FREQ_SEQ_JSON, USX_TEMPLATES
 ...
 * @param[in] len            length in bytes
 * @param[out] out           output buffer - should be large enough to hold compressed output
 * @param[in] olen           length of 'out' buffer in bytes. Can be omitted if sufficient buffer is
 * provided
 ...
 */
extern int unishox2_compress(const char *in,
                             int len,
 ...
 * @param[in] usx_templates  Templates of frequently occurring patterns. See USX_TEMPLATES macro.
 */
extern int unishox2_decompress(const char *in,
                               int len,
                               UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                               const unsigned char usx_hcodes[],
                               const unsigned char usx_hcode_lens[],
 ...
 */
extern int unishox2_compress_lines(const char *in,
                                   int len,
                                   UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                                   const unsigned char usx_hcodes[],
                                   const unsigned char usx_hcode_lens[],
 ...
 * decompressed.
 */
extern int unishox2_decompress_lines(const char *in,
                                     int len,
                                     UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                                     const unsigned char usx_hcodes[],
                                     const unsigned char usx_hcode_lens[],
