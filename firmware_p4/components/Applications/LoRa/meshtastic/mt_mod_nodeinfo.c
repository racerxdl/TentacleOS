...
void mt_mod_nodeinfo_init(uint32_t node_num) {
  s_node_num = node_num;
  s_initial_broadcast_at_s =
      MT_NODEINFO_INITIAL_MIN_S + (esp_random() % MT_NODEINFO_INITIAL_JITTER_S);
  ...
  ESP_LOGI(TAG,
           "Initialized - '%s' / '%s' (initial broadcast in %lus)",
           s_long_name,
           s_short_name,
...
void mt_mod_nodeinfo_on_received(const mt_packet_meta_t *meta,
                                 ...
                                 uint16_t len) {
  ESP_LOGI(TAG, "NodeInfo received from 0x%08lX (%u bytes)", (unsigned long)meta->from, len);

  bool was_unknown =
      (meta->from != 0 && meta->from != s_node_num && mt_nodedb_get(meta->from) == NULL);

  if (meta->from != 0 && meta->from != s_node_num && payload != NULL && len > 0) {
    uint8_t hops =
        (meta->hop_start >= meta->hop_limit) ? (uint8_t)(meta->hop_start - meta->hop_limit) : 0;
    mt_nodedb_upsert_from_user(meta->from, payload, len, (float)meta->snr_db, meta->rssi_dbm, hops);
  }
  ...
  ESP_LOGI(TAG,
           "%s NodeInfo -> 0x%08lX req_id=0x%08lX",
...
void mt_mod_nodeinfo_tick(uint32_t now_s) {
  ...
  if (no_peers && s_broadcast_count > 0 && s_broadcast_count < MT_NODEINFO_BOOT_BURST_COUNT) {
    uint32_t burst =
        MT_NODEINFO_BOOT_BURST_MIN_S + (esp_random() % MT_NODEINFO_BOOT_BURST_JITTER_S);
    if (interval == 0 || burst < interval)
      interval = burst;
  ...
  if (s_last_broadcast_s == 0) {
    if (now_s < s_initial_broadcast_at_s)
      return;
  } else {
    if (interval == 0)
      return;
    if (now_s - s_last_broadcast_s < interval)
      return;
  }
  ...
  uint8_t user[96];
  uint16_t ulen = build_user(user);
  ESP_LOGI(TAG, "Broadcast NodeInfo #%u: '%s' (%s)", s_broadcast_count, s_long_name, s_short_name);
  meshtastic_mesh_send_data(MT_NODEINFO_BCAST_ADDR,
                            0,
                            MT_NODEINFO_HOP_LIMIT,
                            MT_PORT_NODEINFO,
                            user,
...
void mt_mod_nodeinfo_request(uint32_t to) {
  if (to == 0 || to == s_node_num || to == MT_NODEINFO_BCAST_ADDR)
    return;
  ...
  for (int i = 0; i < MT_NODEINFO_REQUEST_SLOTS; i++) {
    ...
    if (s_req_slots[i].peer == 0 && free_slot < 0)
      free_slot = i;
...
send: {
  uint8_t user[96];
  uint16_t ulen = build_user(user);
  ESP_LOGI(TAG, "NodeInfo request -> 0x%08lX (want_response)", (unsigned long)to);
  meshtastic_mesh_send_data(
      to, 0, MT_NODEINFO_HOP_LIMIT, MT_PORT_NODEINFO, user, ulen, 0, false, true);
}
}
