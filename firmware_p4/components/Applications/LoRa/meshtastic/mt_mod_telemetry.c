...
void mt_mod_telemetry_on_received(const mt_packet_meta_t *meta,
                                  ...
                                  uint16_t len) {
  (void)payload;
  ESP_LOGI(TAG, "Telemetry received from 0x%08lX (%u bytes)", (unsigned long)meta->from, len);
...
void mt_mod_telemetry_tick(uint32_t now_s) {
  ...
  meshtastic_mesh_send_data(
      MT_TELEM_BCAST_ADDR, 0, MT_TELEM_HOP_LIMIT, MT_PORT_TELEMETRY, frame, flen, 0, false, false);
}
