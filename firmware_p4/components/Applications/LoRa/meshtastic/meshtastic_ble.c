...
static void notify_fromnum(void) {
  if (!s_is_connected || s_fromnum_handle == 0)
    return;
  s_fromnum_counter++;
  struct os_mbuf *om = ble_hs_mbuf_from_flat(&s_fromnum_counter, sizeof(s_fromnum_counter));
  if (om != NULL) {
...
static void notify_task(void *arg) {
  ...
  while (1) {
    ...
    bool has_data = phoneapi_has_data();
    if (has_data && !last_had_data) {
      notify_fromnum();
    }
    last_had_data = has_data ? 1 : 0;
  }
}
...
on_fromradio_access(uint16_t conn, uint16_t attr, struct ble_gatt_access_ctxt *ctxt, void *arg) {
  ...
  uint8_t frame[MT_BLE_FROMRADIO_BUFSZ];
  uint16_t flen = phoneapi_poll_fromradio(frame, sizeof(frame));
  if (flen > 0) {
    os_mbuf_append(ctxt->om, frame, flen);
...
on_toradio_access(uint16_t conn, uint16_t attr, struct ble_gatt_access_ctxt *ctxt, void *arg) {
  ...
  if (len == 0 || len > MT_BLE_FROMRADIO_BUFSZ)
    return 0;
  uint8_t buf[MT_BLE_FROMRADIO_BUFSZ];
  os_mbuf_copydata(ctxt->om, 0, len, buf);

  ESP_LOGI(TAG, "TORADIO WRITE %u bytes", len);
  phoneapi_on_toradio(buf, len);
...
esp_err_t meshtastic_ble_init(uint32_t node_num) {
  ...
  char name[32];
  snprintf(name, sizeof(name), "Meshtastic_%04lx", (unsigned long)(node_num & 0xFFFF));
  ...
  xTaskCreate(
      notify_task, "ble_notify", MT_BLE_NOTIFY_TASK_SZ, NULL, tskIDLE_PRIORITY + 2, &s_notify_task);
