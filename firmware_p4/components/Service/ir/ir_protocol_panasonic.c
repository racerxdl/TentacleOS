...
bool panasonic_decode(rmt_symbol_word_t *symbols, size_t count, ir_data_t *out) {
  ...
  // 48 bits LSB first
  uint64_t raw =
      ir_decode_pulse_distance(symbols, 1, 48, PANASONIC_ONE_SPACE, PANASONIC_ZERO_SPACE, false);
...
size_t panasonic_encode(const ir_data_t *data, rmt_symbol_word_t *symbols, size_t max) {
  ...
  uint64_t raw = (uint64_t)vendor | ((uint64_t)byte0 << 16) | ((uint64_t)byte1 << 24) |
                 ((uint64_t)byte2 << 32) | ((uint64_t)byte3 << 40);

  return ir_encode_pulse_distance(symbols,
                                  PANASONIC_HEADER_MARK,
                                  PANASONIC_HEADER_SPACE,
                                  PANASONIC_BIT_MARK,
                                  PANASONIC_ONE_SPACE,
