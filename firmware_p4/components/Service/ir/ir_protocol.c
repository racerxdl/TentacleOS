...
size_t ir_encode_pulse_distance(rmt_symbol_word_t *symbols,
  ...
  if (header_mark > 0) {
    symbols[idx].duration0 = header_mark;
    symbols[idx].level0 = 1;
    symbols[idx].duration1 = header_space;
    symbols[idx].level1 = 0;
    idx++;
  }

  for (size_t i = 0; i < num_bits; i++) {
    int bit = msb_first ? (data >> (num_bits - 1 - i)) & 1 : (data >> i) & 1;
    symbols[idx].duration0 = bit_mark;
    symbols[idx].level0 = 1;
    symbols[idx].duration1 = bit ? one_space : zero_space;
    symbols[idx].level1 = 0;
    idx++;
  }
...
size_t ir_encode_pulse_width(rmt_symbol_word_t *symbols,
  ...
  for (size_t i = 0; i < num_bits; i++) {
    int bit = msb_first ? (data >> (num_bits - 1 - i)) & 1 : (data >> i) & 1;
    symbols[idx].duration0 = bit ? one_mark : zero_mark;
    symbols[idx].level0 = 1;
    symbols[idx].duration1 = bit_space;
    symbols[idx].level1 = 0;
    idx++;
  }

  if (stop_bit) {
    symbols[idx].duration0 = zero_mark;
    symbols[idx].level0 = 1;
    symbols[idx].duration1 = 0;
    symbols[idx].level1 = 0;
    idx++;
...
bool ir_decode(rmt_symbol_word_t *symbols, size_t count, ir_data_t *out) {
  ...
  if (nec_decode(symbols, count, out))
    return true;
  if (lg_decode(symbols, count, out))
    return true;
  if (jvc_decode(symbols, count, out))
    return true;
  if (samsung_decode(symbols, count, out))
    return true;
  if (panasonic_decode(symbols, count, out))
    return true;
  if (rc6_decode(symbols, count, out))
    return true;
  if (sony_decode(symbols, count, out))
    return true;

  // Headerless protocols last
  if (rc5_decode(symbols, count, out))
    return true;
  if (denon_decode(symbols, count, out))
    return true;
...
size_t ir_encode(const ir_data_t *data, rmt_symbol_word_t *symbols, size_t max) {
  switch (data->protocol) {
    case IR_PROTO_NEC:
      return nec_encode(data, symbols, max);
    case IR_PROTO_SAMSUNG:
      return samsung_encode(data, symbols, max);
    case IR_PROTO_RC6:
      return rc6_encode(data, symbols, max);
    case IR_PROTO_RC5:
      return rc5_encode(data, symbols, max);
    case IR_PROTO_SONY:
      return sony_encode(data, symbols, max);
    case IR_PROTO_LG:
      return lg_encode(data, symbols, max);
    case IR_PROTO_JVC:
      return jvc_encode(data, symbols, max);
    case IR_PROTO_DENON:
      return denon_encode(data, symbols, max);
    case IR_PROTO_PANASONIC:
      return panasonic_encode(data, symbols, max);
    default:
      return 0;
  }
}
