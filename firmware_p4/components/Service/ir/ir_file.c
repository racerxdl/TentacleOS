...
static const char *ir_to_flipper_proto(ir_protocol_t proto, uint16_t address, uint16_t command) {
  switch (proto) {
    case IR_PROTO_NEC:
      return (address > 0xFF) ? "NECext" : "NEC";
    case IR_PROTO_SAMSUNG:
      return "Samsung32";
    case IR_PROTO_RC6:
      return "RC6";
    case IR_PROTO_RC5:
      return (command > 0x3F) ? "RC5X" : "RC5";
    ...
    case IR_PROTO_SONY:
      if (address > 0xFF)
        ...
        return "SIRC";
    default:
      return NULL; // no Flipper equivalent, use raw
  }
}
...
static void finalize_signal(ir_signal_t *current,
...
size_t ir_file_to_string(const ir_file_t *file, char *buf, size_t buf_size) {
  ...
  for (size_t i = 0; i < file->count; i++) {
    ...
    if (sig->is_raw) {
      ...
      const char *proto =
          ir_to_flipper_proto(sig->data.protocol, sig->data.address, sig->data.command);
      if (proto != NULL) {
