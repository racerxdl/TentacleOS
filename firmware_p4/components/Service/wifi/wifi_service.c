...
void wifi_init(void) {
  spi_bridge_send_command(SPI_ID_WIFI_START, NULL, 0, NULL, NULL, 2000);
}

void wifi_service_scan(void) {
  spi_bridge_send_command(SPI_ID_WIFI_SCAN, NULL, 0, NULL, NULL, 20000);
}
...
wifi_ap_record_t *wifi_service_get_ap_record(uint16_t index) {
  spi_header_t resp;
  if (spi_bridge_send_command(
          SPI_ID_SYSTEM_DATA, (uint8_t *)&index, 2, &resp, (uint8_t *)&cached_record, 1000) ==
      ESP_OK) {
...
esp_err_t wifi_service_connect_to_ap(const char *ssid, const char *password) {
  ...
  return spi_bridge_send_command(
      SPI_ID_WIFI_CONNECT, (uint8_t *)&cfg, sizeof(cfg), NULL, NULL, 15000);
}

bool wifi_service_is_connected(void) {
  spi_header_t resp;
  spi_system_status_t sys = {0};
  if (spi_bridge_send_command(SPI_ID_SYSTEM_STATUS, NULL, 0, &resp, (uint8_t *)&sys, 1000) ==
      ESP_OK) {
...
const char *wifi_service_get_connected_ssid(void) {
  spi_header_t resp;
  if (spi_bridge_send_command(
          SPI_ID_WIFI_GET_STA_INFO, NULL, 0, &resp, (uint8_t *)connected_ssid, 1000) == ESP_OK) {
    connected_ssid[32] = '\0';
...
void wifi_stop(void) {
  spi_bridge_send_command(SPI_ID_WIFI_STOP, NULL, 0, NULL, NULL, 2000);
}
esp_err_t wifi_set_ap_ssid(const char *ssid) {
  return spi_bridge_send_command(
      SPI_ID_WIFI_SET_AP, (uint8_t *)ssid, strlen(ssid), NULL, NULL, 2000);
}
...
esp_err_t wifi_save_ap_config(
  ...
  return spi_bridge_send_command(
      SPI_ID_WIFI_SAVE_AP_CONFIG, (uint8_t *)&cfg, sizeof(cfg), NULL, NULL, 2000);
}
...
esp_err_t wifi_set_ap_password(const char *password) {
  if (!password)
    return ESP_ERR_INVALID_ARG;
  return spi_bridge_send_command(
      SPI_ID_WIFI_SET_AP_PASSWORD, (uint8_t *)password, strlen(password), NULL, NULL, 2000);
}
...
esp_err_t wifi_set_ap_ip(const char *ip_addr) {
  if (!ip_addr)
    return ESP_ERR_INVALID_ARG;
  return spi_bridge_send_command(
      SPI_ID_WIFI_SET_AP_IP, (uint8_t *)ip_addr, strlen(ip_addr), NULL, NULL, 2000);
}
