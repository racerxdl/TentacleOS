...
static void bt_sniffer_stream_cb(spi_id_t id, const uint8_t *payload, uint8_t len) {
  (void)id;
  if (!sniffer_cb || !payload || len < sizeof(spi_ble_sniffer_frame_t))
    return;
...
static bool bluetooth_get_info(spi_bt_info_t *out_info) {
  if (!out_info)
    return false;
  spi_header_t resp;
  if (spi_bridge_send_command(SPI_ID_BT_GET_INFO, NULL, 0, &resp, (uint8_t *)out_info, 1000) ==
      ESP_OK) {
    return true;
  }
  return false;
...
esp_err_t bluetooth_save_announce_config(const char *name, uint8_t max_conn) {
  ...
  return spi_bridge_send_command(
      SPI_ID_BT_SAVE_ANNOUNCE_CFG, (uint8_t *)&cfg, sizeof(cfg), NULL, NULL, 2000);
}
...
esp_err_t bluetooth_load_spam_list(char ***list, size_t *count) {
  ...
  *count = 0;
  ...
  for (uint16_t i = 0; i < total; i++) {
    ...
    if (spi_bridge_send_command(SPI_ID_SYSTEM_DATA, (uint8_t *)&idx, 2, &resp, item_buf, 1000) !=
        ESP_OK) {
...
esp_err_t bluetooth_save_spam_list(const char *const *list, size_t count) {
  ...
  esp_err_t err = spi_bridge_send_command(
      SPI_ID_BT_SPAM_LIST_BEGIN, (uint8_t *)&total, sizeof(total), NULL, NULL, 2000);
  if (err != ESP_OK)
    return err;
  ...
  for (uint16_t i = 0; i < total; i++) {
    ...
    err = spi_bridge_send_command(
        SPI_ID_BT_SPAM_LIST_ITEM, payload, sizeof(payload), NULL, NULL, 2000);
    if (err != ESP_OK)
      return err;
  }
...
ble_scan_result_t *bluetooth_service_get_scan_result(uint16_t index) {
  spi_header_t resp;
  if (spi_bridge_send_command(
          SPI_ID_SYSTEM_DATA, (uint8_t *)&index, 2, &resp, (uint8_t *)&cached_scan_record, 1000) ==
      ESP_OK) {
