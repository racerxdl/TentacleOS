  ...
  *out_resp_len = 0;
  ...
  switch (id) {
    ...
    case SPI_ID_WIFI_SET_ENABLED:
      if (len < 1)
        return SPI_STATUS_INVALID_ARG;
      return (wifi_set_wifi_enabled(payload[0] ? true : false) == ESP_OK) ? SPI_STATUS_OK
                                                                          : SPI_STATUS_ERROR;
    ...
    case SPI_ID_WIFI_APP_PROBE_MON:
      if (probe_monitor_start()) {
        ...
        spi_bridge_provide_results_dynamic(
            results, probe_monitor_get_count_ptr(), sizeof(probe_record_t));
        return SPI_STATUS_OK;
      }
      return SPI_STATUS_BUSY;
    ...
    case SPI_ID_WIFI_TARGET_SCAN_START: {
      ...
      if (target_scanner_start(bssid, channel)) {
        ...
        spi_bridge_provide_results_dynamic(
            results, target_scanner_get_count_ptr(), sizeof(target_client_record_t));
        return SPI_STATUS_OK;
      }
      return SPI_STATUS_ERROR;
    }
