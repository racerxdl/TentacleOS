  ...
  *out_resp_len = 0;
  ...
  switch (id) {
    ...
    case SPI_ID_BT_CONNECT: {
      ...
      return (bluetooth_service_connect(payload, payload[6], NULL) == ESP_OK) ? SPI_STATUS_OK
                                                                              : SPI_STATUS_ERROR;
    }
    ...
    case SPI_ID_BT_APP_FLOOD: {
      if (len < 7)
        return SPI_STATUS_ERROR;
      return (ble_connect_flood_start(payload, payload[6]) == ESP_OK) ? SPI_STATUS_OK
                                                                      : SPI_STATUS_ERROR;
    }
