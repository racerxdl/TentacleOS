...
static void free_task_memory() {
  if (spam_task_stack) {
    heap_caps_free(spam_task_stack);
    spam_task_stack = NULL;
  }
  if (spam_task_tcb) {
    heap_caps_free(spam_task_tcb);
    spam_task_tcb = NULL;
  }
}
...
bool beacon_spam_start_custom(const char *json_path) {
  ...
  spam_task_stack = (StackType_t *)heap_caps_malloc(BEACON_SPAM_STACK_SIZE * sizeof(StackType_t),
                                                    MALLOC_CAP_SPIRAM);
  spam_task_tcb =
      (StaticTask_t *)heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);

  if (spam_task_stack && spam_task_tcb) {
    spam_task_handle = xTaskCreateStatic(beacon_spam_task,
                                         "beacon_spam",
