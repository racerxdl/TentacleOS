...
static void target_scanner_task(void *pvParameters) {
  ...
  scan_results = (target_client_record_t *)heap_caps_malloc(
      max_scan_results * sizeof(target_client_record_t), MALLOC_CAP_SPIRAM);
  if (scan_results == NULL) {
...
bool target_scanner_start(const uint8_t *target_bssid, uint8_t channel) {
  ...
  is_scanning = true;
  scanner_task_handle = xTaskCreateStatic(target_scanner_task,
                                          "tgt_scan_task",
                                          SCANNER_STACK_SIZE,
                                          NULL,
                                          5,
