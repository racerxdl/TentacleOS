...
void signal_monitor_start(const uint8_t *bssid, uint8_t channel) {
  ...
  if (monitor_task_stack == NULL) {
    monitor_task_stack = (StackType_t *)heap_caps_malloc(MONITOR_STACK_SIZE * sizeof(StackType_t),
                                                         MALLOC_CAP_SPIRAM);
  }
  if (monitor_task_tcb == NULL) {
    monitor_task_tcb = (StaticTask_t *)heap_caps_malloc(sizeof(StaticTask_t),
  ...
  monitor_task_handle = xTaskCreateStatic(signal_monitor_task,
                                          "sig_mon_task",
                                          MONITOR_STACK_SIZE,
                                          NULL,
                                          5,
